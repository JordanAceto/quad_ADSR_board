...
#
#   The attack LUT is a truncated rising curve, and the deacy LUT is a decaying
#   curve. The decay LUT is used for both the decay and release phases.
#
...
# generate the attack curve, range [0, MAX_VALUE], increasing
y_a = 1 - np.exp(-X / ATTACK_TARGET)
y_a = y_a - y_a.min() # start at zero
y_a = y_a / y_a.max() # range [0, 1]
y_a = y_a * MAX_VALUE # range [0, MAX_VALUE]

# generate the decay/release curve, range [0, MAX_VALUE], decreasing
y_d = np.exp(-X)
y_d = y_d - y_d.min() # end at zero
y_d = y_d / y_d.max() # range [0, 1]
y_d = y_d * MAX_VALUE # range [0, MAX_VALUE]
...
ATTACK_TABLE_TYPE = f'const uint32_t ADSR_ATTACK_TABLE[{LUT_SIZE}]'
DECAY_TABLE_TYPE = f'const uint32_t ADSR_DECAY_TABLE[{LUT_SIZE}]'
